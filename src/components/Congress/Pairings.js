      ...
      <table className="w-full divide-y divide-slate-900 table-auto border-slate-900 border shadow-lg">
        ...
        <tbody className="bg-slate-800">
          ...
          {pairings.map((p, key) => {
            ...
            const whitePlayer = players.find(player => player.chessResulsSeed === p[0]);
            const blackPlayer = players.find(player => player.chessResulsSeed === p[1]);
...
function Bye() {
  return (<td className="flex-grow-0 max-w-full px-2 pl-4 sm:px-4 py-2 whitespace-nowrap text-center text-xs sm:text-md sm:font-medium text-white">BYE</td>)
}