      ...
      <table className="w-full divide-y divide-slate-900 table-auto border-slate-900 border shadow-lg">
        <thead className="bg-cyan-700">
          <tr>
            <th
              scope="col"
              className="px-1 py-2 text-center text-xs font-medium text-cyan-400 uppercase tracking-wider"
            >Brd.
            </th>
            <th
              scope="col"
          ...
          {pairings.map((p, key) => {
            ...
            return (
              <tr key={key} className={isEven ? "bg-slate-800" : "bg-slate-900"}>
                <td className={isEven ? "bg-slate-800 px-2 py-3 border-r border-slate-700 text-xs text-white" 
